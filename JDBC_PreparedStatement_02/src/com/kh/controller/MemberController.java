...
//				dao로부터 반환받은 결과에 따라서 성공인지 실패인지 판다나 후 응답화면 결정(View 매소드 호출)
public class MemberController {
	 ...
	 */
	public void insertMember(String userId,String userPsw,String userName,String gender,String age,String email, 
		...
		Member m = new Member(userId, userPsw, userName, gender, Integer.parseInt(age), email, 
				phone,address, hobby);
		...
		if(result > 0) { // insert가 성공적으로 완료되었구나
			...
			new MemberMenu().displayFail("회원추가를 실패햐였습니다");
		}
	...
	public void selectByUserName(String keyword) {
		...
		if(list.size() > 0) { //리스트에 검색결과가 담겨 있을 때
			new MemberMenu().displayMemberList(list);
		}else { // 검색 결과가 비어있을 때
			new MemberMenu().displayNoData(keyword + "로 검색된 결과가 없습니다.");
		}
	}
	...
	public void updateMember(String userId,String userPwd,
		...
		if(result > 0) {
			new MemberMenu().displaySuccess(userId+"성공적으로 회원 정보 변경되었습니다.");
		} else {
			new MemberMenu().displayFail(userId +"원 정보 변경실패");
	...
	public void deleteMember(String userId) {
		...
		if(result > 0) {
			new MemberMenu().displaySuccess("성공적으로 회원 탈퇴 처리 하였습니다.");
		}else {
